...
for (year in 2022:2025) {
  ...
  for (i in 1:length(zip_name)) {
    ...
    files <- zip::zip_list(destfile)
    csv_files  <-  files |> 
      filter(str_detect(filename, fixed(".CSV")))
    ...
    ### if the folder extracted is called CSV then change it 
    folder <- paste0(raw, "/", year )
    new_name <- sub("\\.zip$", "", zip_name[i])
    
    old_path <- file.path(folder, "CSV")
